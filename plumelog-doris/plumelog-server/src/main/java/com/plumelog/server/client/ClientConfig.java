...
public class ClientConfig implements InitializingBean {
    ...
    public AbstractServerClient initAbstractServerClient() {
        if (InitConfig.DORIS_CLIENT_NAME.equals(client)) {
            return getDorisLowerClient();
        } else if (InitConfig.ELASTIC_CLIENT_NAME.equals(client)) {
            return getElasticLowerClient();
        }
        return null;
    }

    private DorisLowerClient getDorisLowerClient() {
        return new DorisLowerClient();
    }

    private ElasticLowerClient getElasticLowerClient() {
