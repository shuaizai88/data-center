  ...
  <div class="pnl_wraper">
    <div style="padding: 20px;border-radius: 10px;background: #fff">
      ...
      <div class="pnl_filters">
          ...
          <div style="width: 1000px">
            ...
            <table class='tbl_filters' style="width: 100%">
              ...
              <tr>
                ...
                <td style="width: 180px">
                  <Input class="txt" style="width: 162px" name="serverName" v-model="filter.serverName"
                         placeholder="服务器Ip或者host"
                         :clearable="true"/>
                </td>
              </tr>
              </tbody>
            ...
            <table class="tbl_filters" style="width:1000px">
              <tr v-if="extendList.length>0">
                ...
                <td>
                  <Select v-model="select_extend" @on-change="selectExtendHandler" label-in-value
                          placeholder="选择扩展字段" style="width:150px;margin-right:10px">
                    <Option v-for="extend in extendList" :value="extend.field" :key="extend.field">{{
                        extend.fieldName
                      }}
                    </Option>
                  </Select>
                  <Input class="txt" @on-enter="addExtendTag" :clearable="true" v-model="extendTag"
                         placeholder="输入查询内容" style="width:478px;"/>
        ...
        <div style="clear:both"></div>
        <div v-if="showFilter" style="height: 1px;background:  rgb(247, 247, 247);margin: 10px 0;"></div>
      </div>
      ...
      <div class="pnl_content">
        ...
        <div v-if="tableModel">
          ...
          <Table size="small" border highlight-row :columns="showColumns" :content="self" @on-row-dblclick="dblclick"
            ...
            <template slot-scope="{ row }" slot="traceId">
              <span style="color: #2d8cf0;cursor: pointer;" @click="doSearchByTraceId(row)">{{ row.traceId }}</span>
        ...
        <div v-else :style="this.darkMode && !this.autoWordWrap ? 'padding:0 0 0 10px' : 'padding:0 10px'">
          <div class="log_model_normal"
               :class="[autoWordWrap ? 'auto_word_wrap' : 'word_inline', darkMode ? 'dark_mode' : 'bright_mode']">
              ...
              <div class="log_model_normal_row_selector row_over_flow_hidden">
                <Tooltip placement="top-start" delay="1500">
                  ...
                  <span v-if="showTheColumn('logLevel')" class="row_underline row_search"
                        @click="doSearch('logLevel', row)" :title="'点击查询日志等级: ' + row.logLevel">{{
                      row.logLevel
                    }}</span>
                  <span v-if="showTheColumn('appName')" class="row_app_name row_underline row_search"
                        @click="doSearch('appName',row)" :title="'点击查询应用名称: ' + row.appName">{{
                      row.appName
                    }}</span>
                  ...
                  <span v-if="row.traceId && showTheColumn('traceId')" class="row_trace_id row_underline row_search"
                        @click="doSearchByTraceId(row)" :title="'点击查询TraceId: ' + row.traceId">{{
                      row.traceId
                    }}</span>
                  <span v-if="showTheColumn('className')" class="row_class_name row_underline row_search"
                        @click="doSearch('className', row)"
      ...
      <nav class="page_nav" aria-label="Page navigation example">
        <div class="pnl_select">
          ...
          <Select v-model="showColumnTitles" multiple placeholder="选择要显示的字段" @on-change="columnsChange"
                  :max-tag-count="2"
                  style="width:270px">
        ...
        <div class="pnl_select" style="margin-left:20px">
          <Button @click="autoWordWrapChange" v-if="!tableModel && autoWordWrap" style="font-size:12px">切换至单行模式
          </Button>
          <Button @click="autoWordWrapChange" v-if="!tableModel && !autoWordWrap" style="font-size:12px">
            切换至自动换行
          </Button>
        </div>
        ...
        <ul v-if="totalCount" class="pagination justify-content-center"
          ...
          <li class="page-item">
            <div class="page-count">第{{ parseInt(from / size) + 1 }}页 /
              共{{ parseInt(totalCount / size) + (totalCount % size === 0 ? 0 : 1) }}页
            </div>
          </li>
          <li class="page-item">
...
export default {
  ...
  methods: {
    queryParams() {
      ...
      if (search) {
        ...
        for (let pair of pairs) {
          ...
          // 处理多值参数（如 key=1&key=2）
          params[decodeURIComponent(key)] =
              (params[decodeURIComponent(key)] || []).concat(decodeURIComponent(value || ''));
        }
      }
      return params;
    },
    ...
    getQueryParams() {
      if (this.queryParams()) {
        if (this.queryParams().traceId) {
          this.filter.traceId = this.queryParams().traceId
        }
        if (this.queryParams().time) {
          const parts = this.queryParams().time[0].split(',');
          let startTime = this.formatTime(Number(parts[0]))
          let endTime = this.formatTime(Number(parts[1]))
    ...
    analysisAppNameWithEnv() {
      ...
      this.allAppNames.sort(function (a, b) {
      ...
      this.allEnvs.sort(function (a, b) {
        return a.localeCompare(b)
      });
      this.envComplete = this.allEnvs;
    },
    ...
    doSearchByTraceId(item) {
      ...
      this.filter.traceId = item.traceId
      ...
      let searchUrl = url + '&size=' + this.size + "&from=" + this.from + '&type=table';
      ...
      axios.post(process.env.VUE_APP_API + '/clientQuery?clientStartDate=' + Date.parse(this.dateTimeRange[0])
          + '&clientEndDate=' + Date.parse(this.dateTimeRange[1]) + '&from=0&size=0&type=chartData&chartData', chartFilter).then(data => {
    ...
    getErrorRate() {
      ...
      let url = process.env.VUE_APP_API + '/clientQuery?clientStartDate=' + Date.parse(this.dateTimeRange[0])
          + '&clientEndDate=' + Date.parse(this.dateTimeRange[1]) + '&from=0&size=0&type=errChat&errChat';
